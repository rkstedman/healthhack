module.exports = function(app) {
  ...
  Controller.show = function(req, res) {
    ...
    PatientModel.read(patientId, function(error, patient) {
      patient.profile.age = Math.round(((new Date()).getTime() + patient.profile.birthdate.getTime())/(24 * 3600 * 365.25 * 1000));
      res.format({
        html: function(){
          if(error) {
            res.render('patients/patientList', {error: error});
  ...
  Controller.print = function(req, res) {
    // default to patient view if none specified
    var view = req.query.view || 'clinician';
    var patientId = req.params.id;
    PatientModel.read(patientId, function(error, patient) {
      ...
      res.format({
        html: function(){
          if(error) {
            ...
            res.render('patients/patientPrintView', {patient: patient, view: view});
          }
        }, 
        json: function(){
          if(error) {
